...
class Robot2():
	...
	def handle_map_message(self, message):
		...
		for i in range(self.config["map_size"][0]):
			...
			for j in range(self.config["map_size"][1]):
				if message.data[self.config["map_size"][1] * i + j] == "2":
					self.cur_pos = [i, j]
				elif message.data[self.config["map_size"][1] * i + j] == "1":
					self.opp_pos = [i, j]
	...
	def handle_turn(self, message):
		...
		self.cur_pos = path[1]
		self.board[self.cur_pos[0]][self.cur_pos[1]] = "2"

		if self.cur_pos == self.goal:
			self.reached_goal_pub.publish(True)

		new_obstacle = [-999999, []]
		for i in range(self.opp_pos[0] + 3, self.opp_pos[0] - 3, -1):
			if i < 0 or i >= self.config["map_size"][0]:
				continue
			for j in range(self.opp_pos[1] + 3, self.opp_pos[1] - 3, -1):
				...
				#if opp_path == False or len(opp_path) <= new_obstacle[0] or AStarSearch(self.cur_pos, self.goal, self.board) == False:
		...
		if new_obstacle[1] == []:
			...
			self.board[new_obstacle[1][0]][new_obstacle[1][1]] = "WALL"
			self.map_publisher.publish(PolicyList(self.to_policy()))
